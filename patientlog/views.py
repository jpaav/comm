...
def log_detail(request, log_id, entry_id):
	...
	try:
		...
		return render(request, 'patientlogs/object_does_not_exist.html', {'obj_type': 'log'})
	# Properly filter entries depending on the contents of the url
	# Second argument is default value
	filter = request.GET.get('filter', 'newest')
	offset = int(request.GET.get('offset', '0'))
	count = Entry.objects.filter(log=log).count()
	if count < 100:
		offset = 0
	...
	return render(request, 'patientlogs/log.html', {'log': log, 'entries': entries, 'detail': detail, 'filter': filter,
													'offset': offset})
